...
from models import db, User, Profile, UserSetting, Chat, Message, MessageReaction, Contact, Call, Notification, Report, AuditLog

app = Flask(__name__)
app.config['SECRET_KEY'] = 'convo-cave-enterprise-encryption-secret'

# Load Database URL from Environment Variable (Fallback to SQLite for local PC testing)
db_url = os.environ.get('DATABASE_URL', 'sqlite:///convocave.db')

# Automatic fix: SQLAlchemy 1.4+ requires "postgresql://" instead of "postgres://"
...
def signup():
    ...
    if request.method == 'POST':
        ...
        new_user = User(username=username, email=email, phone=phone if phone else None, password_hash=hashed_pw, is_admin=is_first)
        db.session.add(new_user)
        db.session.commit()
        
        user_profile = Profile(user_id=new_user.id, first_name=username, country=country, language=language, timezone=timezone, bio=bio)
        user_settings = UserSetting(user_id=new_user.id)
        db.session.add(user_profile)
        db.session.add(user_settings)
        db.session.commit()
        
        log = AuditLog(user_id=new_user.id, action="account_created", ip_address=request.remote_addr)
        db.session.add(log)
        db.session.commit()
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password):
            if user.is_banned or user.is_suspended:
                flash('Your account access has been revoked.')
                return redirect(url_for('login'))
            login_user(user)
            log = AuditLog(user_id=user.id, action="login_success", ip_address=request.remote_addr)
            db.session.add(log)
            db.session.commit()
            return redirect(url_for('dashboard'))
        else:
...
@login_required
def dashboard():
    main_chat = Chat.query.filter_by(name="# Global Sanctuary").first()
    if not main_chat:
        main_chat = Chat(name="# Global Sanctuary", is_group=True, group_description="The primary public communication hub.")
        db.session.add(main_chat)
        db.session.commit()
        
    all_rooms = Chat.query.filter_by(is_group=True).all()
    all_users = User.query.filter(User.id != current_user.id).all()
    
    return render_template('dashboard.html', rooms=all_rooms, users=all_users, default_room=main_chat)
